...
import { Upload, FileText, CheckCircle, XCircle, Clock, Camera, Eye, Trash2 } from "lucide-react"
...
export function EnhancedKYCVerification({ currentStatus = "not_started" }: KYCVerificationProps) {
  const { t } = useI18n()
  ...
  const handleFileUpload = async (documentId: string, file: File) => {
    const result = await uploadFile(file, {
      ...
      maxSize: 10 * 1024 * 1024, // 10MB for KYC documents
    })
            ...
            <div key={document.id} className="border rounded-lg p-4">
                ...
                <div className="flex-1">
                  ...
                  {document.fileName && (
                    <p className="text-xs text-muted-foreground mt-1">
                      File: {document.fileName} • Uploaded {new Date(document.uploadedAt!).toLocaleDateString()}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                {document.uploaded && document.fileUrl && (
                  <>
                    <Button variant="ghost" size="sm" onClick={() => handleViewDocument(document.fileUrl!)}>
